...
1. **Schutzbedarf bestimmen: Nehmt eure Liste der Zielobjekte aus der vorherigen Lern-Story. Bestimmt nun für jedes Zielobjekt den Schutzbedarf für V, I und V. Nutzt eine Tabelle mit den Spalten:** `Zielobjekt`**,** `Schutzbedarf Vertraulichkeit`**,** `Begründung`**,** `Schutzbedarf Integrität`**,** `Begründung`**,** `Schutzbedarf Verfügbarkeit`**,** `Begründung`**.**
  ...
  | **NAS-Server** | Hoch | Zwischengespeicherte Dateien können vertraulich sein. | Hoch | Daten müssen fehlerfrei gespeichert werden, und über die gesamte Lebensdauer korrekt, konsistent und vertrauenswürdig bleiben. | Hoch | Daten müssen für Arbeitsfluss verfügbar sein, und so bereitstehen, dass sie jeder Zeit vom Arbeitsprozess genutzt werden |
